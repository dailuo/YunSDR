...
% Different Tx and/or Rx diversity options require different processing of the pilot symbols
   ...
   phase_error_est = angle(sum(conj(ref_pilots).*squeeze(rx_pilots).*conj(scramble_patt)));
   correction_phases = zeros(sim_consts.NumDataSubc, num_symbols);
   correction_phases = repmat(phase_error, sim_consts.NumDataSubc, 1);   
end
